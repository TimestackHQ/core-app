...
export default function EditEvent() {
	...
	const [cover, setCover] = React.useState<any>(route.params.eventThumbnail);
	...
	const importCover = async () => {
		...
		ImagePicker.launchImageLibrary({
			...
			if (!result.didCancel) {
				...
				const snapshot = media?.type === "video" ? await generateScreenshot(mediaId + "snapshot", uri) : null
	...
	return <KeyboardAwareScrollView innerRef={ref => [scrollRef]} style={{ backgroundColor: "white", paddingTop: 20 }}>
		<View style={{ justifyContent: 'center', alignItems: 'flex-end', padding: 10, paddingTop: 0 }}>
			<TouchableOpacity disabled={loadingCover} onPress={updateEvent}>
				<Text style={{
					color: loadingCover ? "grey" : "black",
					fontSize: 15,
					fontFamily: "Red Hat Display Semi Bold",
				}}>Save</Text>
			</TouchableOpacity>
		...
		<View style={{ backgroundColor: "white", flexDirection: "column" }}>
			<View style={{ flexDirection: "row", flex: 1 }}>
				...
				<View style={{ flex: 2, marginRight: 5 }}>
					<Text style={{ fontFamily: 'Red Hat Display Semi Bold', marginBottom: -5, color: "gray", }}>Title </Text>
					<TextInput
						multiline={true}
						value={name}
